...
Before the first CPU dataset run, test Jamendo's individual-track API with a few hundred selected IDs. A four-format, one-track pilot is complete; [the report](reports/audio_format_pilot.json) records downloads, temporary-file deletion, decoding, preprocessing, inference time, and representation similarity. OGG is the current low-size choice (about 1.50 MB per 124-second track; its 384-value feature had 0.9968 cosine similarity to FLAC), while MP32 is closest to FLAC (about 3.15 MB; 0.9991 cosine). This is a representation comparison for one track, not evidence of downstream label accuracy; check allowed-download coverage and task metrics before freezing the training input. The feature cache is about 6.2 MiB for 4,250 single-window examples. If per-track API access cannot cover the chosen IDs, stream MTG TAR archives one at a time and extract only selected files; this limits peak disk to an archive plus extracted targets, but transfers the full archive collection. MTG lists the full low-quality collection at 156 GB. [Jamendo track download API](https://developer.jamendo.com/v3.0/tracks/file), [MTG audio options](https://github.com/MTG/mtg-jamendo-dataset#downloading-the-data).
...
`scripts/audio_format_pilot.py` compares Jamendo `mp31`, `mp32`, OGG, and FLAC on one permitted track and deletes each temporary audio file after processing. To reproduce it, set `JAMENDO_CLIENT_ID` for the process, then run `python scripts/audio_format_pilot.py`; it uses the installed `ffmpeg`/`ffprobe` commands and the pinned local backbone.
