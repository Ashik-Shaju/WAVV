...
Extract OGG one track at a time, delete it immediately after feature storage, and carry the old features into a separate 4,250-row SQLite cache only after metadata and row alignment checks pass. The measured 10-track run projects about 6.6 hours for the 3,450 new rows; it projects about 8.1 hours only if all 4,250 had to be fetched. Complete and audit Gate 2A, then stop. Training and test evaluation remain a later step.
