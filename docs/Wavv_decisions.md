...
Prefer human consensus labels for covered tasks. Reuse the same selected audio IDs' MTG uploader tags for broader weak-tag tasks, including instruments, so one audio download serves both. Use Jamendo's per-track OGG API for eligible selected tracks, decode to mono 32 kHz through the pinned DyMN04-AS frontend, and delete each temporary audio file after feature extraction. Verify MTG-to-Jamendo ID matching, per-track download permission, and class/artist coverage before freezing the manifest. Start with a few-hundred-track timing/acquisition pilot. Then grow the fit partition only as needed, with validation-only data scaling until the smallest subset passing the per-task support and validation gates is reached; lock final test artists throughout. Do not claim a fixed sample cap yields validated quality. Do not fall back to the full `audio-low` archives for the initial constrained run; defer tasks if per-track OGG coverage is inadequate. Keep DyMN04-AS frozen.

**Reproducible comparison:** with the same track/artist split, OGG input, deterministic 10-second window policy, frozen pooled features, and linear-head baseline, record per-taxonomy metrics and support at successive training-set sizes. The five-track codec pilot supports OGG for the next low-space extraction run based on representation similarity and size only; it does not establish listening quality or downstream task accuracy. Keep human and uploader-tag scores separate because their label definitions differ. Compare each task with its prevalence/majority baseline; expand only where validation or class support fails. Freeze the TSV-derived label contract and artist-grouped manifests before downloading beyond the timing pilot.

## D-044 — Start with the measured 800-track human-label baseline
**Status:** Superseded by D-046

For the initial CPU-constrained baseline, use the completed 800 eligible annotated IDs as the seed cohort with an artist-disjoint 560/120/120 split. It was the smallest tested cap that retained all eight human attribute tasks. This remains historical evidence and is superseded as the final data cap by D-046.

The 2026-09-24 Jamendo inventory recorded 4,915 download-allowed IDs, 228 explicit denials, and 5,528 IDs with no metadata record; missing records are not denials. Bind the frozen manifest to that exact snapshot. The live metadata endpoint intermittently returned empty batches on a repeated audit, so do not repeat that query as a hard preflight gate. The per-track audio-file endpoint remains the final permission check; if it rejects a frozen ID, stop and refresh/rebuild rather than substitute a different recording.

## D-045 — Use a frozen-feature logistic baseline before fine-tuning
**Status:** Accepted

After the complete 800-row feature cache passes Gate 2A, train one independent binary logistic head for each retained human taxonomy on the frozen 384-D DyMN04-AS pooled feature. Use task-labeled fit rows only, fit feature scaling on fit data only, and use unweighted BCE-with-logits plus L2 regularization. Choose regularization and threshold on validation; reserve the coverage-balanced test cohort for one locked evaluation. Report per-class tracks/artists and artist-cluster uncertainty. This is the first-fit recipe, not a claim that the tasks already meet product-quality thresholds.

This replaces D-043's few-hundred-track pre-download timing pilot: first use a 9-track end-to-end smoke check, then extract the full selected 800-track baseline and record its actual timing/bytes. The 9-track check verified deterministic finite 384-value pooled features, exact 10-second mono 32 kHz windows, and temporary audio cleanup. Complete the feature cache for all 800 IDs before training. The 9-track and five-codec pilots support the extraction mechanics and OGG representation choice only; they do not validate downstream task accuracy.

## D-046 — Expand the final pre-training cohort to 4,250 tracks and reuse the 800-track seed
**Status:** Accepted

Use the deterministic 4,250-track artist-disjoint manifest (2,975 fit, 638 validation, 637 test; 302/65/65 artist groups) for the one planned model fit. Preserve the exact prior 800 rows and their split/artist/Jamendo-ID mapping, and acquire only the remaining 3,450 tracks. The manifest retains the same eight human-consensus tasks and adds five genre plus four instrument uploader-tag targets whose positive and negative track/artist counts meet the reporting floors in every split. Keep weak-label metrics separate; a missing uploader tag means “not tagged,” not verified acoustic absence. Gender and tonal/atonal still fail the support gate. This is a support-based training plan, not evidence of downstream accuracy or a natural-prevalence test sample.

Extract OGG one track at a time, delete it immediately after feature storage, and carry the old features into a separate 4,250-row SQLite cache only after metadata and row alignment checks pass. The measured 10-track run projects about 6.6 hours for the 3,450 new rows; it projects about 8.1 hours only if all 4,250 had to be fetched. The first live 53 new rows took about nine minutes, so the current early-rate estimate is roughly 9.7 hours remaining and will vary with network conditions. Complete and audit Gate 2A, then stop. Training and test evaluation remain a later step.
