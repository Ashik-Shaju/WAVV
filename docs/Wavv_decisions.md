...
For the first CPU-constrained Music Understanding baseline, freeze the 800 eligible annotated IDs currently represented by the hash-linked inventory and artist-disjoint 560/120/120 fit/validation/test manifest. This is the smallest tested cap in the final support sweep that retains all eight available human attribute tasks: seven binary MTG mood/acousticness taxonomies plus danceability. The per-class 100-track/20-artist fit and 30-track/10-artist validation/test floors are reporting minimums, not accuracy guarantees. Defer gender and tonal/atonal because rare-class support fails; defer all weak uploader-tag tasks because no label reaches the same floor at this cap. The 4,250-track cap is an optional expansion ceiling only after validation demonstrates a specific need. Do not claim the complete Music Understanding profile is trained by this first baseline; genre, instruments, vocal/instrumental, and valence/arousal remain future tasks until their data contracts pass.
