...
4. Exact and near-duplicate audio checks across MTG-Jamendo and DEAM are not complete because no corpus audio is retained. The split metadata has IDs and paths; its separate raw metadata has title/artist strings. Keep the task datasets separate until audio-level duplicate checks can run.
